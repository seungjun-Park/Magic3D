...
def download_dataset_with_crawling(url, driver_path, download_path='./download', secs=3):
    driver = webdriver.Chrome(driver_path)
    driver.get(url)
    time.sleep(secs)

    elements = driver.find_element(By.CLASS_NAME, 'post-gallery')

    sub = []
    pictures = []

    for i in elements.find_elements(By.CLASS_NAME, 'post-preview'):
        sub.append(i)

    for i in sub:
        print(i.get_attribute('data-tags'))
        picture = i.find_element(By.CSS_SELECTOR, 'img')
...
if __name__ == '__main__':
    download_dataset_with_crawling(url='https://danbooru.donmai.us/', driver_path='C:/Users/dan70/Desktop/chromedriver_win32/chromedriver.exe')