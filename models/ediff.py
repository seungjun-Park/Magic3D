...
class EDiff(pl.LightningModule):
    def __init__(self,
        ...
        self.model_mean_type = ModelMeanType.EPSILON if self.parameterization == 'eps' else ModelMeanType.START_X
        self.model_var_type = ModelVarType.LEARNED_RANGE if learn_sigma else (ModelVarType.FIXED_SMALL if sigma_small else ModelVarType.FIXED_LARGE)
        if use_kl:
            self.loss_type = LossType.RESCALED_KL
        elif rescaled_learned_sigmas:
